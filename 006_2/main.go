...
func page(fname string) *template.Template {
	tmps, er := template.ParseFiles("templates/"+fname+".html", "templates/head.html", "templates/foot.html")
	if er != nil {
		tmps = notemp()
	}
	return tmps
...
func index(w http.ResponseWriter, rq *http.Request) {
	...
	var gl []my.Group
	db.Where("group_id > 0").Order("created_at desc").Limit(10).Find(&gl)
...
func post(w http.ResponseWriter, rq *http.Request) {
	...
	db.Table("comments").Select("comments.*, users.id, users.name").Joins("join users on users.id = comments.users_id").Where("comments.post_id = ?", pid).Order("created_at desc").Find(&cmts)
